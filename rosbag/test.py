...
wamv_x = wamv_x[600:]
time_stamps_wamv = time_stamps_wamv[600:]
ukf_x = ukf_x[600:]
time_stamps_ukf = time_stamps_ukf[600:]
...
fig, ax = plt.subplots()
ax.plot(time_stamps_common, errors, color="blue", label="Error of Y Velocity")
ax.axhline(y=rmse, color='red', linestyle='--', label=f"RMSE ({rmse:.4f} m)")
ax.set_xlabel("Time (s)")
ax.set_ylabel("Error (m)")
ax.set_title("Error Between Target's Y Velocity and Estimated Y Velocity")
ax.legend()
plt.show()

print(f"Overall RMSE in Y Velocity: {rmse:.4f} m")